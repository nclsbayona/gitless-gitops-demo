...
sed -i 's/^cowsay=1\.0\.0/cowsay=1.0.1/' ../definitions/app/versions.txt
git add ../definitions/app/ && git commit -m "feat(definitions): Add new eyes to cowsay"
...
while true; do
	ROLLOUTS=$(kubectl rollout history deployment/cowsay -n dev | grep "2" | wc -l)
	if [ "$ROLLOUTS" -ge 1 ]; then
		break
	fi
